...
export async function DELETE(request: Request) {
  try {
    ...
    const { error: attendanceError } = await supabase.from("attendance").delete().eq("course_id", courseId)
    ...
    const { error: courseStudentsError } = await supabase.from("course_students").delete().eq("course_id", courseId)
    ...
    // 3. Delete sessions
    const { error: sessionsError } = await supabase.from("sessions").delete().eq("course_id", courseId)
    ...
    if (deleteCourseError) {
      ...
      return NextResponse.json({ error: `Error deleting course: ${deleteCourseError.message}` }, { status: 500 })
    }
