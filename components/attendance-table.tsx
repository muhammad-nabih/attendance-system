...
import { CheckCircle, XCircle, Clock, MoreHorizontal, ChevronLeft, ChevronRight } from "lucide-react"
              ...
              <SelectContent>
                {sessions.map((session) => (
                  <SelectItem key={session.id} value={session.id}>
                    محاضرة {session.session_number} - {format(parseISO(session.date), "yyyy/MM/dd", { locale: ar })}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
              ...
              {displayedSessions.map((session) => (
                <th key={session.id} className="p-2 text-center font-medium whitespace-nowrap">
                  ...
                  <span className="text-xs font-normal">
                    {format(parseISO(session.date), "yyyy/MM/dd", { locale: ar })}
                  </span>
                </th>
              ))}
            </tr>
